...
export async function fetchUserMonthlyReportData(firestore: Firestore, userId: string, currentMonth: Date, schoolConfig: any, monthlyConfig: any) {
    ...
    for (const day of allDaysInMonth) {
        ...
        if (attendanceRecord) {
            recordForDay = { id: attendanceRecord.id, date: parseISO(attendanceRecord.date), checkInTime: attendanceRecord.checkInTime?.toDate() || null, checkOutTime: attendanceRecord.checkOutTime?.toDate() || null, status: attendanceRecord.status || 'Hadir', description: attendanceRecord.description || 'Hadir' };
        } else {
