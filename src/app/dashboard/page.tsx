...
import {
  Users,  UserCheck,  UserX,  BookUser,  Loader2,  School, LogIn, LogOut, TrendingUp, AlertCircle, Info, MailWarning, Clock4
} from 'lucide-react';
import {
  Card,
  CardContent,
...
const PersonalAttendanceCardUI = ({ attendanceData, isLoading, lateSubmissionData }: { attendanceData: any, isLoading: boolean, lateSubmissionData: any }) => {
    ...
    const hasPendingLateSubmission = useMemo(() => lateSubmissionData && lateSubmissionData.length > 0, [lateSubmissionData]);
    ...
    const reminder = useMemo(() => {
        ...
        if (isLoading || hasCheckedOut || attendanceWindowStatus === 'HOLIDAY' || hasPendingLateSubmission) {
            return null; 
        }
    ...
    }, [attendanceRecord, isLoading, attendanceWindowStatus, schoolConfigData, currentTime, checkInEnd, checkOutStart, hasPendingLateSubmission]);
    ...
    }, [isLoading, attendanceRecord, schoolConfigData, currentTime, attendanceWindowStatus, checkInEnd, checkOutStart, hasPendingLateSubmission]);
...
const HeadmasterDashboard = ({ user, router }: any) => {
    ...
    const todaysLateSubmissionQuery = useMemoFirebase(() => user ? query(collection(firestore, 'users', user.uid, 'lateSubmissions'), where('date', '==', format(new Date(), 'yyyy-MM-dd')), where('status', '==', 'pending'), limit(1)) : null, [firestore, user]);
